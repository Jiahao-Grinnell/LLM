...
def _should_skip_administrative_page_without_names(current_ocr: str, page_type: str, rule_names: List[Dict[str, str]]) -> bool:
    if rule_names or page_type != "administrative_memo":
        return False
    shortish = len(normalize_ws(current_ocr)) <= 900
    explicit_name_anchor = bool(re.search(
        r"\b(?:named\s+[A-Z]|namely|slave\s*no\.?\s*\d+|statement\s+made\s+by|statement\s+of(?:\s+slave)?|grant\s+.+?manumission\s+certificate)\b",
        current_ocr,
        flags=re.I,
    ))
    return shortish and not explicit_name_anchor


def _merge_person_inventory(existing: Dict[str, Any], candidate: Dict[str, Any]) -> Dict[str, Any]:
    existing["Name"] = choose_preferred_name(existing.get("Name", ""), candidate.get("Name", ""))

    source_names = set(existing.get("source_names") or [])
    source_names.update(candidate.get("source_names") or [])
    existing["source_names"] = sorted(n for n in source_names if n)

    evidences = set(existing.get("name_evidence") or [])
    evidences.update(candidate.get("name_evidence") or [])
    existing["name_evidence"] = sorted(e for e in evidences if e)

    if len(candidate.get("case_ocr", "")) > len(existing.get("case_ocr", "")):
        existing["case_ocr"] = candidate.get("case_ocr", "")
    if len(candidate.get("name_context", "")) > len(existing.get("name_context", "")):
        existing["name_context"] = candidate.get("name_context", "")

    if candidate.get("case_doc_year") and not existing.get("case_doc_year"):
        existing["case_doc_year"] = candidate.get("case_doc_year")

    existing["case_page_type"] = candidate.get("case_page_type") or existing.get("case_page_type") or "unknown"
    existing["case_ocr_quality"] = candidate.get("case_ocr_quality") or existing.get("case_ocr_quality") or "unknown"
    return existing


def finalize_name_inventory_for_page(current_ocr: str, page: int, stats: Dict[str, int], logger: logging.Logger,
                                     ordered_pages: Optional[List[int]] = None, page_texts: Optional[Dict[int, str]] = None) -> Tuple[List[Dict[str, Any]], str, Dict[str, Any]]:
    page_type = classify_document_page(current_ocr)
    if page_type in {"index", "record_metadata"}:
        return [], "skip_index_page", {"page_type": page_type}

    ocr_quality, quality_info = score_ocr_quality(current_ocr)
    if ocr_quality == "garbled" and not quality_info.get("strong_anchor"):
        return [], "skip_bad_ocr", {"page_type": page_type, "ocr_quality": ocr_quality, "quality_info": quality_info}

    doc_year = extract_doc_year(current_ocr)
    rule_names = deterministic_named_slaves(current_ocr)
    if _should_skip_administrative_page_without_names(current_ocr, page_type, rule_names):
        return [], "skip_no_named_slave", {"page_type": page_type, "ocr_quality": ocr_quality, "quality_info": quality_info, "doc_year": doc_year}

    model_names: List[Dict[str, str]] = []
    try:
        model_names = model_named_slaves(current_ocr, stats)
    except Exception as e:
        logger.warning("Page %s name-pass failed: %s", page, e)

    names = merge_named_candidates(rule_names, model_names)
    if not names:
        return [], "skip_no_named_slave", {"page_type": page_type, "ocr_quality": ocr_quality, "quality_info": quality_info, "doc_year": doc_year}
    ...
    for item in names:
        ...
        if not is_likely_personal_name(raw_name):
            continue

        case_ocr = collect_case_context(page, current_ocr, raw_name, ordered_pages or [], page_texts or {})
        canonical_name = canonicalize_name_against_context(raw_name, case_ocr)
        name_context = extract_name_scoped_context(case_ocr, canonical_name) or extract_name_scoped_context(case_ocr, raw_name) or case_ocr
        case_doc_year = extract_doc_year(case_ocr) or doc_year
        case_page_type = classify_document_page(case_ocr)
        case_ocr_quality, _ = score_ocr_quality(case_ocr)

        candidate = {
            "Name": canonical_name,
            "source_names": sorted({raw_name, canonical_name}),
            "name_evidence": [normalize_ws(item.get("evidence", ""))] if normalize_ws(item.get("evidence", "")) else [],
            "case_ocr": case_ocr,
            "name_context": name_context,
            "case_doc_year": case_doc_year,
            "case_page_type": case_page_type,
            "case_ocr_quality": case_ocr_quality,
        }

        merged = False
        for prev in inventory:
            if names_maybe_same_person(prev.get("Name", ""), candidate["Name"]):
                _merge_person_inventory(prev, candidate)
                merged = True
                break
        if not merged:
            inventory.append(candidate)

    inventory = [p for p in inventory if is_likely_personal_name(str(p.get("Name") or ""))]
    if not inventory:
        return [], "skip_no_named_slave", {"page_type": page_type, "ocr_quality": ocr_quality, "quality_info": quality_info, "doc_year": doc_year}
    ...
    return inventory, "ok", meta


def extract_routes_for_inventory(inventory: List[Dict[str, Any]], page: int, stats: Dict[str, int], logger: logging.Logger) -> List[Dict[str, Any]]:
    place_rows: List[Dict[str, Any]] = []
    for person in inventory:
        canonical_name = str(person.get("Name") or "")
        case_ocr = str(person.get("case_ocr") or "")
        case_doc_year = person.get("case_doc_year")
        case_page_type = str(person.get("case_page_type") or "unknown")
        case_ocr_quality = str(person.get("case_ocr_quality") or "unknown")

        allow_route = page_supports_personal_route(case_page_type, case_ocr, case_ocr_quality)
        ...
        places = merge_places(model_places, det_places)
...
def extract_metadata_for_inventory(inventory: List[Dict[str, Any]], page: int, report_type: str, stats: Dict[str, int], logger: logging.Logger) -> List[Dict[str, Any]]:
    detail_rows: List[Dict[str, Any]] = []
    for person in inventory:
        canonical_name = str(person.get("Name") or "")
        name_context = str(person.get("name_context") or person.get("case_ocr") or "")

        base_meta = deterministic_meta(name_context, canonical_name, page, report_type)
        model_meta: Dict[str, Any] = {}
        try:
            model_meta = model_meta_for_name(name_context, canonical_name, page, report_type, stats)
        except Exception as e:
            logger.warning("Page %s meta-pass failed for %s: %s", page, canonical_name, e)
...
def process_page(current_ocr: str, page: int, filename: str, report_type: str, stats: Dict[str, int], logger: logging.Logger,
    ...
    inventory, stage_status, page_meta = finalize_name_inventory_for_page(
    ...
    place_rows = extract_routes_for_inventory(inventory, page, stats, logger)
    detail_rows = extract_metadata_for_inventory(inventory, page, report_type, stats, logger)
